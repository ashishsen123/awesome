...
if ( ! function_exists( 'awesome_setup' ) ) {
	function awesome_setup() {
		 ...
		 */
		load_theme_textdomain( 'awesome', trailingslashit( get_template_directory() ) . 'languages' );
		...
		register_nav_menus( array(
				'primary' => esc_html__( 'Primary Menu', 'awesome' )
			) );
...
function awesome_fonts_url() {
	 ...
	 */
	$pt_sans = _x( 'on', 'PT Sans font: on or off', 'awesome' );
	 ...
	 * Do not translate into your own language.
	 */
	$subset = _x( 'no-subset', 'PT Sans font: add new subset (cyrillic)', 'awesome' );
	 ...
	 * Do not translate into your own language.
	 */
	$arvo = _x( 'on', 'Arvo font: on or off', 'awesome' );
 ...
 */
function awesome_mce_css( $mce_css ) {
	$fonts_url = awesome_fonts_url();
...
add_filter( 'mce_css', 'awesome_mce_css' );
...
function awesome_widgets_init() {
	register_sidebar( array(
			'name' => esc_html__( 'Main Sidebar', 'awesome' ),
			'id' => 'sidebar-main',
			'description' => esc_html__( 'Appears in the sidebar on posts and pages except the optional Front Page template, which has its own widgets', 'awesome' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'First Front Page Banner Widget', 'awesome' ),
			'id' => 'frontpage-banner1',
			'description' => esc_html__( 'Appears in the banner area on the Front Page', 'awesome' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'Second Front Page Banner Widget', 'awesome' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'First Front Page Widget Area', 'awesome' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'Second Front Page Widget Area', 'awesome' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'Third Front Page Widget Area', 'awesome' ),
	...
	register_sidebar( array(
			'name' => esc_html__( 'Fourth Front Page Widget Area', 'awesome' ),
	...
	register_sidebar( array(
			...
			'id' => 'sidebar-footer1',
			'description' => esc_html__( 'Appears in the footer sidebar', 'awesome' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>'
	...
	register_sidebar( array(
			'name' => esc_html__( 'Second Footer Widget Area', 'awesome' ),
			'id' => 'sidebar-footer2',
			'description' => esc_html__( 'Appears in the footer sidebar', 'awesome' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
	...
	register_sidebar( array(
			'name' => esc_html__( 'Third Footer Widget Area', 'awesome' ),
			'id' => 'sidebar-footer3',
			'description' => esc_html__( 'Appears in the footer sidebar', 'awesome' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
	...
	register_sidebar( array(
			'name' => esc_html__( 'Fourth Footer Widget Area', 'awesome' ),
			'id' => 'sidebar-footer4',
			'description' => esc_html__( 'Appears in the footer sidebar', 'awesome' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
 ...
 * @return void
 */
function awesome_scripts_styles() {
	 ...
	 *     wp_dequeue_style( 'awesome-fonts' );
	 * }
	 * add_action( 'wp_enqueue_scripts', 'mytheme_dequeue_fonts', 11 );
	 */
	$fonts_url = awesome_fonts_url();
	if ( !empty( $fonts_url ) ) {
		wp_enqueue_style( 'awesome-fonts', esc_url_raw( $fonts_url ), array(), null );
	}
	...
	if ( is_singular() && comments_open() ) {
		...
		wp_localize_script( 'commentvalidate', 'comments_object', array(
			...
			'author'  => esc_html__( 'Please enter your name', 'awesome' ),
			'email'  => esc_html__( 'Please enter a valid email address', 'awesome' ),
			'comment' => esc_html__( 'Please add a comment', 'awesome' ) )
		);
	}
...
add_action( 'wp_enqueue_scripts', 'awesome_scripts_styles' );
...
function awesome_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'awesome' ), max( $paged, $page ) );
	}
 ...
 * @return void
 */
if ( ! function_exists( 'awesome_content_nav' ) ) {
	function awesome_content_nav( $nav_id ) {
		global $wp_query;
		...
		<nav role="navigation" id="<?php echo $nav_id; ?>" class="<?php echo $nav_class; ?>">
			<h3 class="assistive-text"><?php esc_html_e( 'Post navigation', 'awesome' ); ?></h3>

			<?php if ( is_single() ) { // navigation links for single posts ?>

				<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '<i class="fa fa-angle-left"></i>', 'Previous post link', 'awesome' ) . '</span> %title' ); ?>
				<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '<i class="fa fa-angle-right"></i>', 'Next post link', 'awesome' ) . '</span>' ); ?>
			...
			elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) { // navigation links for home, archive, and search pages ?>
				...
				<?php echo paginate_links( array(
					...
					'type' => 'list',
					'prev_text' => wp_kses( __( '<i class="fa fa-angle-left"></i> Previous', 'awesome' ), array( 'i' => array( 
						'class' => array() ) ) ),
					'next_text' => wp_kses( __( 'Next <i class="fa fa-angle-right"></i>', 'awesome' ), array( 'i' => array( 
						'class' => array() ) ) )
				) ); ?>
 ...
 * simply create your own awesome_comment(), and that function will be used instead.
 *
 ...
 * @return void
 */
if ( ! function_exists( 'awesome_comment' ) ) {
		...
		case 'trackback' :
			...
			<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
				<article id="comment-<?php comment_ID(); ?>" class="pingback">
					<p><?php esc_html_e( 'Pingback:', 'awesome' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'awesome' ), '<span class="edit-link">', '</span>' ); ?></p>
				...
				<article id="comment-<?php comment_ID(); ?>" class="comment">
					<header class="comment-meta comment-author vcard">
						...
						printf( '<cite class="fn">%1$s %2$s</cite>',
							...
							( $comment->user_id === $post->post_author ) ? '<span> ' . esc_html__( 'Post author', 'awesome' ) . '</span>' : '' );
						printf( '<a href="%1$s" title="Posted %2$s"><time itemprop="datePublished" datetime="%3$s">%4$s</time></a>',
							esc_url( get_comment_link( $comment->comment_ID ) ),
							sprintf( esc_html__( '%1$s @ %2$s', 'awesome' ), esc_html( get_comment_date() ), esc_attr( get_comment_time() ) ),
							get_comment_time( 'c' ),
							/* Translators: 1: date, 2: time */
							sprintf( esc_html__( '%1$s at %2$s', 'awesome' ), get_comment_date(), get_comment_time() )
						);
						?>
					</header> <!-- .comment-meta -->

					<?php if ( '0' == $comment->comment_approved ) { ?>
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'awesome' ); ?></p>
					<?php } ?>

					<section class="comment-content comment">
						<?php comment_text(); ?>
						<?php edit_comment_link( esc_html__( 'Edit', 'awesome' ), '<p class="edit-link">', '</p>' ); ?>
					</section> <!-- .comment-content -->

					<div class="reply">
						<?php comment_reply_link( array_merge( $args, array( 'reply_text' => wp_kses( __( 'Reply <span>&darr;</span>', 'awesome' ), array( 'span' => array() ) ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
					</div> <!-- .reply -->
				</article> <!-- #comment-## -->
			<?php
			break;
 ...
 */
function awesome_comment_form_default_fields( $fields ) {

	$commenter = wp_get_current_commenter();
	$req = get_option( 'require_name_email' );
	$aria_req = ( $req ? ' aria-required="true"' : "" );

	$fields[ 'author' ] = '<p class="comment-form-author">' . '<label for="author">' . esc_html__( 'Name', 'awesome' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' . '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>';

	$fields[ 'email' ] =  '<p class="comment-form-email"><label for="email">' . esc_html__( 'Email', 'awesome' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' . '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';

	$fields[ 'url' ] =  '<p class="comment-form-url"><label for="url">' . esc_html__( 'Website', 'awesome' ) . '</label>' . '<input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>';

	return $fields;

}
add_action( 'comment_form_default_fields', 'awesome_comment_form_default_fields' );
 ...
 */
function awesome_comment_form_field_comment( $field ) {

	$field = '<p class="comment-form-comment"><label for="comment">' . _x( 'Comment', 'noun', 'awesome' ) . ' <span class="required">*</span></label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>';

	return $field;

}
add_action( 'comment_form_field_comment', 'awesome_comment_form_field_comment' );
 ...
 * @return void
 */
if ( ! function_exists( 'awesome_posted_on' ) ) {
	function awesome_posted_on() {
		$post_icon = '';
		...
		$date = sprintf( '<i class="fa %1$s"></i> <a href="%2$s" title="Posted %3$s" rel="bookmark"><time class="entry-date" datetime="%4$s" itemprop="datePublished">%5$s</time></a>',
			...
			sprintf( esc_html__( '%1$s @ %2$s', 'awesome' ), esc_html( get_the_date() ), esc_attr( get_the_time() ) ),
		...
		$author = sprintf( '<i class="fa fa-pencil"></i> <address class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></address>',
			...
			esc_attr( sprintf( esc_html__( 'View all posts by %s', 'awesome' ), get_the_author() ) ),
			get_the_author()
		);

		// Return the Categories as a list
		$categories_list = get_the_category_list( esc_html__( ' ', 'awesome' ) );
		...
		$comments = sprintf( '<span class="comments-link"><i class="fa fa-comment"></i> <a href="%1$s" title="%2$s">%3$s</a></span>',
			...
			( get_comments_number() > 0 ? sprintf( _n( '%1$s Comment', '%1$s Comments', get_comments_number() ), get_comments_number() ) : esc_html__( 'No Comments', 'awesome' ) )
		);

		// Translators: 1: Date 2: Author 3: Categories 4: Comments
		printf( wp_kses( __( '<div class="header-meta">%1$s%2$s<span class="post-categories">%3$s</span>%4$s</div>', 'awesome' ), array( 
			'div' => array ( 
				'class' => array() ), 
			'span' => array( 
				'class' => array() ) ) ),
 ...
 * @return void
 */
if ( ! function_exists( 'awesome_entry_meta' ) ) {
	function awesome_entry_meta() {
		// Return the Tags as a list
		$tag_list = "";
		if ( get_the_tag_list() ) {
			$tag_list = get_the_tag_list( '<span class="post-tags">', esc_html__( ' ', 'awesome' ), '</span>' );
		}

		// Translators: 1 is tag
		if ( $tag_list ) {
			printf( wp_kses( __( '<i class="fa fa-tag"></i> %1$s', 'awesome' ), array( 'i' => array( 'class' => array() ) ) ), $tag_list );
		}
	}
}
 ...
 * @return void
 */
function awesome_content_width() {
	if ( is_page_template( 'page-templates/full-width.php' ) || is_attachment() ) {
		global $content_width;
		$content_width = 1200;
	}
}
add_action( 'template_redirect', 'awesome_content_width' );
 ...
 */
function awesome_remove_more_jump_link( $link ) {
...
add_filter( 'the_content_more_link', 'awesome_remove_more_jump_link' );
...
function awesome_continue_reading_link() {
	return '&hellip;<p><a class="more-link" href="'. esc_url( get_permalink() ) . '" title="' . esc_html__( 'Continue reading', 'awesome' ) . ' &lsquo;' . get_the_title() . '&rsquo;">' . wp_kses( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'awesome' ), array( 'span' => array( 
			'class' => array() ) ) ) . '</a></p>';
}


/**
 * Replaces "[...]" (appended to automatically generated excerpts) with the awesome_continue_reading_link().
 *
 * @since Awesome 1.0
 *
...
function awesome_auto_excerpt_more( $more ) {
	return awesome_continue_reading_link();
}
 ...
 */
function awesome_new_contactmethods( $contactmethods ) {
	// Add Twitter
...
add_filter( 'user_contactmethods', 'awesome_new_contactmethods', 10, 1 );
 ...
 */
function awesome_add_menu_parent_class( $items ) {
...
add_filter( 'wp_nav_menu_objects', 'awesome_add_menu_parent_class' );
 ...
 */
if ( ! function_exists( 'awesome_get_social_media' ) ) {
	function awesome_get_social_media() {
		$output = '';
		$icons = array(
			array( 'url' => of_get_option( 'social_twitter', '' ), 'icon' => 'fa-twitter', 'title' => esc_html__( 'Follow me on Twitter', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_facebook', '' ), 'icon' => 'fa-facebook', 'title' => esc_html__( 'Friend me on Facebook', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_googleplus', '' ), 'icon' => 'fa-google-plus', 'title' => esc_html__( 'Connect with me on Google+', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_linkedin', '' ), 'icon' => 'fa-linkedin', 'title' => esc_html__( 'Connect with me on LinkedIn', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_dribbble', '' ), 'icon' => 'fa-dribbble', 'title' => esc_html__( 'Follow me on Dribbble', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_tumblr', '' ), 'icon' => 'fa-tumblr', 'title' => esc_html__( 'Follow me on Tumblr', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_github', '' ), 'icon' => 'fa-github', 'title' => esc_html__( 'Fork me on GitHub', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_bitbucket', '' ), 'icon' => 'fa-bitbucket', 'title' => esc_html__( 'Fork me on Bitbucket', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_foursquare', '' ), 'icon' => 'fa-foursquare', 'title' => esc_html__( 'Follow me on Foursquare', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_youtube', '' ), 'icon' => 'fa-youtube', 'title' => esc_html__( 'Subscribe to me on YouTube', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_instagram', '' ), 'icon' => 'fa-instagram', 'title' => esc_html__( 'Follow me on Instagram', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_flickr', '' ), 'icon' => 'fa-flickr', 'title' => esc_html__( 'Connect with me on Flickr', 'awesome' ) ),
			array( 'url' => of_get_option( 'social_pinterest', '' ), 'icon' => 'fa-pinterest', 'title' => esc_html__( 'Follow me on Pinterest', 'awesome' ) )
		);
 ...
 */
if ( ! function_exists( 'awesome_get_credits' ) ) {
	function awesome_get_credits() {
		$output = '';
		$output = sprintf( '%1$s <a href="%2$s" title="%3$s">%4$s</a>',
			esc_html__( 'Proudly powered by', 'awesome' ),
			esc_url( esc_html__( 'http://wordpress.org/', 'awesome' ) ),
			esc_attr( esc_html__( 'Semantic Personal Publishing Platform', 'awesome' ) ),
			esc_html__( 'WordPress', 'awesome' )
		);
 ...
 * @return void
 */
function awesome_theme_options_styles() {
	$output = '';
...
add_action( 'wp_head', 'awesome_theme_options_styles' );
