...
class Options_Framework_Media_Uploader {
	...
	static function optionsframework_uploader( $_id, $_value, $_desc = '', $_name = '' ) {
		...
		$output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="'.$name.'" value="' . $value . '" placeholder="' . esc_html__('No file chosen', 'awesome') .'" />' . "\n";
		if ( function_exists( 'wp_enqueue_media' ) ) {
			if ( ( $value == '' ) ) {
				$output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . esc_html__( 'Upload', 'awesome' ) . '" />' . "\n";
			} else {
				$output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . esc_html__( 'Remove', 'awesome' ) . '" />' . "\n";
			}
		} else {
			$output .= '<p><i>' . esc_html__( 'Upgrade your version of WordPress for full media support.', 'awesome' ) . '</i></p>';
		}
		...
		if ( $value != '' ) {
			...
			if ( $image ) {
				...
				$title = esc_html__( 'View File', 'awesome' );
	...
	function optionsframework_media_scripts( $hook ) {
		...
		wp_localize_script( 'of-media-uploader', 'optionsframework_l10n', array(
			'upload' => esc_html__( 'Upload', 'awesome' ),
			'remove' => esc_html__( 'Remove', 'awesome' )
		) );
	}
}